...
function tk_settings_page_init() {
    ...
    for ($i = 0; $i <= count($pages) - 1; $i++) {
        if ($i == 0) {
            $settings_page .= add_menu_page($pages[$i]['slug'], ucfirst(tk_theme_name).' Theme Settings', 'edit_theme_options', $pages[$i]['slug'], 'tk_settings_page');
    ...
    ?></h2>
        ...
        jQuery(document).ready(function(){
            ...
            jQuery('#button_test').live('click', function(){
                ...
                jQuery.ajax({
                    type: "POST",
                    url: "<?php echo get_template_directory_uri()?>/custom_function/reset_colors.php"
                }).done(function( ) {
